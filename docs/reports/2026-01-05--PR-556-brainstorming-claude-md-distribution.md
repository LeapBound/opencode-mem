Brainstorming Report: CLAUDE.md Distribution Architecture
  ...
  Potential Claude Code feature request - support command substitution in context config:
  {
    "context": {
      "sources": [
        { "command": "opencode-mem live-context ${CWD}" }
      ]
    }
  }
  Or folder-level .claude-context files containing just:
  exec: opencode-mem live-context .
  ...
  - Timeline data in separate file: opencode-mem-timeline.csv
  - Generated fresh on SessionStart
  - Block Claude from reading timeline file via .claude/settings.local.json: "ignorePaths": ["opencode-mem-timeline.csv"]
  ...
  - Writes opencode-mem-timeline.csv and includes in push
  ...
  - Post-pull hook: opencode-mem sync --from-timeline
  ...
  - opencode-mem-timeline.csv = Machine-generated context sync (blocked from reading, already injected)
