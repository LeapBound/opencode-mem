# Issue #543 Analysis: /opencode-mem Slash Command Not Available Despite Installation
...
A user reports that the `/opencode-mem diagnostics` command returns "Unknown slash command: opencode-mem" after installing opencode-mem v8.5.9 on Windows.
...
### Finding 1: No `/opencode-mem` Slash Command Exists

**Critical Discovery**: The `/opencode-mem diagnostics` command does not exist in opencode-mem. After extensive codebase analysis:
...
2. **Skills, not commands**: Claude-mem uses Claude Code's **skill system**, not the command system. Skills are defined in `plugin/skills/`:
   ...
   - `opencode-mem-settings/` - Settings management
...
The troubleshoot skill is designed to be **invoked naturally** - users describe their problem to Claude, and the skill auto-invokes. There is no `/opencode-mem diagnostics` command.
...
The user's issue is **not** that `/opencode-mem diagnostics` doesn't work - that command never existed. The actual issues are:
...
```
This will create `~/.opencode-mem/settings.json` with defaults.

### Step 5: For Diagnostics - Natural Language
Instead of `/opencode-mem diagnostics`, describe the issue to Claude:
> "I'm having issues with opencode-mem. Can you help troubleshoot?"
...
Create a `/opencode-mem` command for diagnostics. File: `plugin/commands/opencode-mem.json`:
```json
{
  "name": "opencode-mem",
...
```javascript
const settingsPath = join(homedir(), '.opencode-mem', 'settings.json');
if (!existsSync(settingsPath)) {
  mkdirSync(join(homedir(), '.opencode-mem'), { recursive: true });
...
if (process.platform === 'win32' && !workerStarted) {
  ...
  console.error('And check: https://docs.opencode-mem.ai/troubleshooting');
}
```
...
1. `/opencode-mem diagnostics` does not exist - users should use natural language to invoke the troubleshoot skill
...
The fix should include both user documentation improvements and potentially adding a `/opencode-mem` diagnostic command for discoverability.
