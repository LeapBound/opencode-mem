...
**Date:** 2026-01-07
**Issue:** [#596](https://github.com/thedotmack/opencode-mem/issues/596)
...
After a clean install of opencode-mem v9.0.0, the SDK agent aborts every observation with a "ProcessTransport is not ready for writing" error. The worker starts successfully and the HTTP API responds, but no observations are stored to the database. The error originates from the Claude Agent SDK's internal transport layer, specifically in the bundled `worker-service.cjs` at line 1119.
...
- **Node:** v22.21.1
- **Claude Code:** 2.0.75
- **opencode-mem:** v9.0.0 (clean install)
...
1. **Parent process:** The opencode-mem worker service
...
The Claude Agent SDK version (`^0.1.76`) may have introduced changes to ProcessTransport initialization timing that conflict with how opencode-mem invokes `query()`.

Evidence:
- v9.0.0 works for some users but fails for others
- Error occurs in SDK internals, not opencode-mem code
...
```bash
# Check worker logs
tail -f ~/.opencode-mem/logs/worker-$(date +%Y-%m-%d).log
