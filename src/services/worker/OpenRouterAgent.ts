...
export class OpenRouterAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      ...
      if (!apiKey) {
        throw new Error('OpenRouter API key not configured. Set OPENCODE_MEM_OPENROUTER_API_KEY in settings or OPENROUTER_API_KEY environment variable.');
      }
  ...
  private truncateHistory(history: ConversationMessage[]): ConversationMessage[] {
    ...
    const MAX_CONTEXT_MESSAGES = parseInt(settings.OPENCODE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES) || DEFAULT_MAX_CONTEXT_MESSAGES;
    const MAX_ESTIMATED_TOKENS = parseInt(settings.OPENCODE_MEM_OPENROUTER_MAX_TOKENS) || DEFAULT_MAX_ESTIMATED_TOKENS;
  ...
  private async queryOpenRouterMultiTurn(
    ...
    const response = await fetch(OPENROUTER_API_URL, {
      ...
      headers: {
        ...
        'HTTP-Referer': siteUrl || 'https://github.com/thedotmack/opencode-mem',
        'X-Title': appName || 'opencode-mem',
  ...
  private getOpenRouterConfig(): { apiKey: string; model: string; siteUrl?: string; appName?: string } {
    ...
    const model = settings.OPENCODE_MEM_OPENROUTER_MODEL || 'xiaomi/mimo-v2-flash:free';

    // Optional analytics headers
    const siteUrl = settings.OPENCODE_MEM_OPENROUTER_SITE_URL || '';
    const appName = settings.OPENCODE_MEM_OPENROUTER_APP_NAME || 'opencode-mem';
...
export function isOpenRouterSelected(): boolean {
  const settingsPath = USER_SETTINGS_PATH;
  const settings = SettingsDefaultsManager.loadFromFile(settingsPath);
  return settings.OPENCODE_MEM_PROVIDER === 'openrouter';
}
