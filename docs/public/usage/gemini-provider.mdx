...
|---------|--------|---------|-------------|
| `OPENCODE_MEM_PROVIDER` | `claude`, `gemini` | `claude` | AI provider for observation extraction |
| `OPENCODE_MEM_GEMINI_API_KEY` | string | — | Your Gemini API key |
| `OPENCODE_MEM_GEMINI_MODEL` | `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash-preview` | `gemini-2.5-flash-lite` | Gemini model to use |
| `OPENCODE_MEM_GEMINI_BILLING_ENABLED` | `true`, `false` | `false` | Skip rate limiting if billing is enabled on Google Cloud |
...
```json
{
  ...
  "OPENCODE_MEM_GEMINI_API_KEY": "your-api-key-here",
  "OPENCODE_MEM_GEMINI_MODEL": "gemini-2.5-flash-lite",
  "OPENCODE_MEM_GEMINI_BILLING_ENABLED": "true"
}
```
...
If Gemini is selected but encounters errors, opencode-mem automatically falls back to the Claude Agent SDK:
...
Either:
- Set `OPENCODE_MEM_GEMINI_API_KEY` in `~/.opencode-mem/settings.json`, or
...
<Tip>
**Recommended**: Enable billing on your Google Cloud project to unlock much higher rate limits. You won't be charged unless you exceed the generous free quota. This allows opencode-mem to process observations instantly instead of waiting between requests.
</Tip>
