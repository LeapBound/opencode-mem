...
description: "Common issues and solutions for OpenCode-Mem"
---
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('opencode-mem-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('opencode-mem-settings')
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.opencode-mem/opencode-mem.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.opencode-mem/opencode-mem.db "SELECT COUNT(*) FROM observations;"
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.opencode-mem/opencode-mem.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.opencode-mem/opencode-mem.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.opencode-mem/opencode-mem.db "VACUUM;"
   ```
...
2. Check for stale locks:
   ```bash
   lsof ~/.opencode-mem/opencode-mem.db
   ```
...
2. Backup database:
   ```bash
   cp ~/.opencode-mem/opencode-mem.db ~/.opencode-mem/opencode-mem.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.opencode-mem/opencode-mem.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.opencode-mem/opencode-mem.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.opencode-mem/opencode-mem.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install opencode-mem` fails.
...
**Solution**: Check `~/.opencode-mem/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/thedotmack/opencode-mem/issues)
...
- [Configuration](configuration) - Customize OpenCode-Mem
