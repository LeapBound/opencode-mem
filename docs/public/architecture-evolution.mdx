...
description: "How opencode-mem evolved from v3 to v5+"
---
...
This is the story of how opencode-mem evolved from a simple idea to a production-ready system, and the key architectural decisions that made it work.
...
const ThemeProvider = ({ children }) => {
  const [theme, setTheme] = useState<'light' | 'dark' | 'system'>(() => {
    return localStorage.getItem('opencode-mem-theme') || 'system';
  });
...
**Impact**: Cross-platform compatibility restored, Windows users can now use opencode-mem without issues.
...
```bash
cp ~/.opencode-mem/opencode-mem.db ~/.opencode-mem/opencode-mem-v3-backup.db
```
