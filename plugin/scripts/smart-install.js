#!/usr/bin/env node
/**
 * Smart Install Script for opencode-mem
 *
...
/**
 * Add shell alias for opencode-mem command
 */
function installCLI() {
  const WORKER_CLI = join(ROOT, 'plugin', 'scripts', 'worker-service.cjs');
  const bunPath = getBunPath() || 'bun';
  const aliasLine = `alias opencode-mem='${bunPath} "${WORKER_CLI}"'`;
  ...
  try {
    if (IS_WINDOWS) {
      ...
      const functionDef = `function opencode-mem { & "${bunPath}" "${WORKER_CLI}" $args }\n`;
      ...
      if (!existingContent.includes('function opencode-mem')) {
        writeFileSync(profilePath, existingContent + '\n' + functionDef);
        console.error(`✅ PowerShell function added to profile`);
        console.error('   Restart your terminal to use: opencode-mem <command>');
      }
    } else {
      ...
      for (const config of shellConfigs) {
        if (existsSync(config)) {
          ...
          if (!content.includes('alias opencode-mem=')) {
...
try {
  ...
  if (needsInstall()) {
    ...
    const port = process.env.OPENCODE_MEM_WORKER_PORT || 37777;
    console.error(`[opencode-mem] Plugin updated to v${newVersion} - restarting worker...`);
    try {
