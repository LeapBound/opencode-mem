...
- **Port**: Fixed port 37777 (configurable via `OPENCODE_MEM_WORKER_PORT`)
- **Location**: `src/services/worker-service.ts`
- **Built Output**: `plugin/scripts/worker-service.cjs`
- **Model**: Configurable via `OPENCODE_MEM_MODEL` environment variable (default: sonnet)
...
- PID file tracking at `~/.opencode-mem/worker.pid`
...
```bash
export OPENCODE_MEM_MODEL=sonnet
```
...
- **Default**: Port 37777
- **Override**: Set `OPENCODE_MEM_WORKER_PORT` environment variable
...
```
~/.opencode-mem/
├── opencode-mem.db           # SQLite database (bun:sqlite)
