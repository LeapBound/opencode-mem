...
**Memory Sources:** 100+ observations from opencode-mem

## Executive Summary

The opencode-mem project has faced significant Windows platform challenges, requiring extensive architectural changes and ongoing maintenance. The issues fall into four major categories:
...
**Issue:** The hooks don't set `OPENCODE_MEM_MANAGED=true` environment variable, so the managed restart code path (lines 314-330 of worker-service.ts) is never activated. Every session runs the "standalone Windows" code path which lacks proper serialization.
...
**The Problem (Dec 28):** On Windows, the `.opencode-mem` directory can be in flux during filesystem operations, causing `worker.lock` file access to fail with ENOENT.
...
5. **Set OPENCODE_MEM_MANAGED=true** in hooks.json to activate managed mode
...
### Documentation
- https://docs.opencode-mem.ai/troubleshooting/windows-issues
