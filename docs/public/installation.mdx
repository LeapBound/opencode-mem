---
title: "Installation"
description: "Install OpenCode-Mem plugin for persistent memory across sessions"
---
...
Install OpenCode-Mem directly from the plugin marketplace:
...
Data is stored in `~/.opencode-mem/`:
- Database: `~/.opencode-mem/opencode-mem.db`
- PID file: `~/.opencode-mem/.worker.pid`
- Port file: `~/.opencode-mem/.worker.port`
- Logs: `~/.opencode-mem/logs/worker-YYYY-MM-DD.log`
- Settings: `~/.opencode-mem/settings.json`

Override with environment variable:
```bash
export OPENCODE_MEM_DATA_DIR=/custom/path
```
...
See [CHANGELOG](https://github.com/thedotmack/opencode-mem/blob/main/CHANGELOG.md) for complete version history.

## Next Steps

- [Getting Started Guide](usage/getting-started) - Learn how OpenCode-Mem works automatically
- [MCP Search Tools](usage/search-tools) - Query your project history
- [Configuration](configuration) - Customize OpenCode-Mem behavior
