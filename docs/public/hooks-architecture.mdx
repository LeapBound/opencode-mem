# How OpenCode-Mem Uses Hooks: A Lifecycle-Driven Architecture
...
OpenCode-Mem is fundamentally a **hook-driven system**. Every piece of functionality happens in response to lifecycle events:
...
**Key insight:** OpenCode-Mem doesn't interrupt or modify Claude Code's behavior. It observes from the outside and provides value through lifecycle hooks.
...
OpenCode-Mem had several architectural constraints:
...
OpenCode-Mem uses lifecycle hook scripts across 5 lifecycle events. SessionStart runs 3 hooks in sequence: smart-install, worker-service start, and context-hook.
...
- ✅ Configurable observation count via `OPENCODE_MEM_CONTEXT_OBSERVATIONS`

**Output format:**
```markdown
# [opencode-mem] recent context
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.opencode-mem/opencode-mem.db "SELECT * FROM observation_queue"`
...
**Privacy guarantees:**
- All data stored locally in `~/.opencode-mem/opencode-mem.db`
...
*The hook-driven architecture enables OpenCode-Mem to be both powerful and invisible. Users never notice the memory system working - it just makes Claude smarter over time.*
