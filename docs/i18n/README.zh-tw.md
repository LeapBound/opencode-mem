...
<p align="center">
  OpenCode-Mem 透過自動擷取工具使用觀察、產生語意摘要並在未來的工作階段中提供使用，無縫保留跨工作階段的脈絡。這使 Claude 即使在工作階段結束或重新連線後，仍能維持對專案的知識連續性。
</p>
...
- **[安裝指南](https://docs.opencode-mem.ai/installation)** - 快速開始與進階安裝
- **[使用指南](https://docs.opencode-mem.ai/usage/getting-started)** - OpenCode-Mem 如何自動運作
- **[搜尋工具](https://docs.opencode-mem.ai/usage/search-tools)** - 使用自然語言查詢專案歷史
- **[Beta 功能](https://docs.opencode-mem.ai/beta-features)** - 試用 Endless Mode 等實驗性功能

### 最佳實務

- **[脈絡工程](https://docs.opencode-mem.ai/context-engineering)** - AI 代理脈絡最佳化原則
- **[漸進式揭露](https://docs.opencode-mem.ai/progressive-disclosure)** - OpenCode-Mem 脈絡啟動策略背後的理念

### 架構

- **[概覽](https://docs.opencode-mem.ai/architecture/overview)** - 系統元件與資料流程
- **[架構演進](https://docs.opencode-mem.ai/architecture-evolution)** - 從 v3 到 v5 的旅程
- **[Hooks 架構](https://docs.opencode-mem.ai/hooks-architecture)** - OpenCode-Mem 如何使用生命週期掛鉤
- **[Hooks 參考](https://docs.opencode-mem.ai/architecture/hooks)** - 7 個掛鉤腳本說明
- **[Worker 服務](https://docs.opencode-mem.ai/architecture/worker-service)** - HTTP API 與 Bun 管理
- **[資料庫](https://docs.opencode-mem.ai/architecture/database)** - SQLite 結構描述與 FTS5 搜尋
- **[搜尋架構](https://docs.opencode-mem.ai/architecture/search-architecture)** - 使用 Chroma 向量資料庫的混合搜尋

### 設定與開發

- **[設定](https://docs.opencode-mem.ai/configuration)** - 環境變數與設定
- **[開發](https://docs.opencode-mem.ai/development)** - 建置、測試、貢獻
- **[疑難排解](https://docs.opencode-mem.ai/troubleshooting)** - 常見問題與解決方案
...
詳情請參閱[架構概覽](https://docs.opencode-mem.ai/architecture/overview)。

---

## MCP 搜尋工具

OpenCode-Mem 透過遵循 Token 高效的 **3 層工作流程模式**，以 **4 個 MCP 工具**提供智慧記憶搜尋：
...
詳細範例請參閱[搜尋工具指南](https://docs.opencode-mem.ai/usage/search-tools)。

---

## Beta 功能

OpenCode-Mem 提供具有實驗性功能的 **Beta 通道**，例如 **Endless Mode**（用於延長工作階段的仿生記憶架構）。在 http://localhost:37777 → Settings 的網頁檢視介面中切換穩定版與 Beta 版。

有關 Endless Mode 與如何試用的詳情，請參閱 **[Beta 功能文件](https://docs.opencode-mem.ai/beta-features)**。
...
設定在 `~/.opencode-mem/settings.json` 中管理（首次執行時自動以預設值建立）。設定 AI 模型、Worker 連接埠、資料目錄、日誌層級與脈絡注入設定。

所有可用設定與範例請參閱 **[設定指南](https://docs.opencode-mem.ai/configuration)**。

---

## 開發

建置說明、測試與貢獻工作流程請參閱 **[開發指南](https://docs.opencode-mem.ai/development)**。
...
常見問題與解決方案請參閱 **[疑難排解指南](https://docs.opencode-mem.ai/troubleshooting)**。
...
貢獻工作流程請參閱[開發指南](https://docs.opencode-mem.ai/development)。
...
- **儲存庫**：[github.com/thedotmack/opencode-mem](https://github.com/thedotmack/opencode-mem)
