---
title: "Configuration"
description: "Environment variables and settings for OpenCode-Mem"
---
...
|-------------------------------|---------------------------------|---------------------------------------|
| `OPENCODE_MEM_MODEL`            | `sonnet`                        | AI model for processing observations (when using Claude) |
| `OPENCODE_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, or `openrouter` |
| `OPENCODE_MEM_MODE`             | `code`                          | Active mode profile (e.g., `code--es`, `email-investigation`) |
| `OPENCODE_MEM_CONTEXT_OBSERVATIONS` | `50`                        | Number of observations to inject      |
| `OPENCODE_MEM_WORKER_PORT`      | `37777`                         | Worker service port                   |
| `OPENCODE_MEM_WORKER_HOST`      | `127.0.0.1`                     | Worker service host address           |
| `OPENCODE_MEM_SKIP_TOOLS`       | `ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion` | Comma-separated tools to exclude from observations |

### Gemini Provider Settings

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `OPENCODE_MEM_GEMINI_API_KEY`   | —                               | Gemini API key ([get free key](https://aistudio.google.com/app/apikey)) |
| `OPENCODE_MEM_GEMINI_MODEL`     | `gemini-2.5-flash-lite`          | Gemini model: `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash-preview` |
...
| Setting                                      | Default                     | Description                           |
|----------------------------------------------|-----------------------------|---------------------------------------|
| `OPENCODE_MEM_OPENROUTER_API_KEY`              | —                           | OpenRouter API key ([get key](https://openrouter.ai/keys)) |
| `OPENCODE_MEM_OPENROUTER_MODEL`                | `xiaomi/mimo-v2-flash:free` | Model identifier (supports 100+ models) |
| `OPENCODE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES` | `20`                        | Max messages in conversation history  |
| `OPENCODE_MEM_OPENROUTER_MAX_TOKENS`           | `100000`                    | Token budget safety limit             |
| `OPENCODE_MEM_OPENROUTER_SITE_URL`             | —                           | Optional: URL for analytics           |
| `OPENCODE_MEM_OPENROUTER_APP_NAME`             | `opencode-mem`                | Optional: App name for analytics      |
...
| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `OPENCODE_MEM_DATA_DIR`         | `~/.opencode-mem`                 | Data directory location               |
| `OPENCODE_MEM_LOG_LEVEL`        | `INFO`                          | Log verbosity (DEBUG, INFO, WARN, ERROR, SILENT) |
| `OPENCODE_MEM_PYTHON_VERSION`   | `3.13`                          | Python version for chroma-mcp         |
...
```bash
./opencode-mem-settings.sh
```

This script manages settings in `~/.opencode-mem/settings.json`.
...
| Setting | Default | Description |
|---------|---------|-------------|
| `OPENCODE_MEM_MODE` | `code` | Defines behavior and language. See [Modes & Languages](modes). |
...
```json
{
  "OPENCODE_MEM_MODE": "email-investigation"
}
```
...
- **Production (installed plugin)**: `~/.opencode-mem/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `OPENCODE_MEM_DATA_DIR`

```
~/.opencode-mem/
├── opencode-mem.db           # SQLite database
...
OpenCode-Mem provides MCP search tools for querying your project history.
...
OpenCode-Mem supports switching between stable and beta versions via the web viewer UI.
...
OpenCode-Mem injects past observations into each new session, giving Claude awareness of recent work. You can configure exactly what gets injected using the **Context Settings Modal**.
...
Settings are stored in `~/.opencode-mem/settings.json`:

```json
{
  "OPENCODE_MEM_CONTEXT_OBSERVATIONS": "100",
  "OPENCODE_MEM_CONTEXT_SESSION_COUNT": "20",
  "OPENCODE_MEM_CONTEXT_OBSERVATION_TYPES": "bugfix,decision,discovery",
  "OPENCODE_MEM_CONTEXT_OBSERVATION_CONCEPTS": "how-it-works,gotcha",
  "OPENCODE_MEM_CONTEXT_FULL_COUNT": "10",
  ...
  "OPENCODE_MEM_CONTEXT_SHOW_LAST_SUMMARY": "false",
...
Settings can be customized in `~/.opencode-mem/settings.json`.
...
```json
{
  "OPENCODE_MEM_DATA_DIR": "/custom/path"
}
```
...
```json
{
  "OPENCODE_MEM_WORKER_PORT": "38000"
}
```
...
```json
{
  "OPENCODE_MEM_MODEL": "opus"
}
```

Then restart the worker:
```bash
export OPENCODE_MEM_MODEL=opus
npm run worker:restart
```

### Custom Skip Tools

Control which tools are excluded from observations. Edit `~/.opencode-mem/settings.json`:
```json
{
  "OPENCODE_MEM_SKIP_TOOLS": "ListMcpResourcesTool,SlashCommand,Skill"
}
```
...
2. Verify environment variables:
   ```bash
   echo $OPENCODE_MEM_MODEL
   echo $OPENCODE_MEM_WORKER_PORT
   ```
...
1. Set custom port:
   ```bash
   export OPENCODE_MEM_WORKER_PORT=38000
   ```
