# Plan: Integrate Workflow Agents and Commands into OpenCode-Mem

## Executive Summary

This plan integrates the `/make-plan` and `/do` orchestration workflow from `~/.claude/commands/` into the opencode-mem plugin as project-level development tools.
...
- Existing example: `.claude/skills/version-bump/SKILL.md` for opencode-mem project tools
...
This directory contains slash commands **for developing and maintaining the opencode-mem project itself**.
...
Add project-level orchestration commands for opencode-mem development:
