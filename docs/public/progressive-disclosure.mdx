# Progressive Disclosure: OpenCode-Mem's Context Priming Philosophy
...
## OpenCode-Mem's Solution: Progressive Disclosure
...
```markdown
*Use opencode-mem MCP search to access records with the given ID*
```
...
OpenCode-Mem implements progressive disclosure through a 3-layer workflow pattern:
...
- [OpenCode-Mem Architecture](architecture/overview) - How it all fits together
...
*This philosophy emerged from real-world usage of OpenCode-Mem across hundreds of coding sessions. The pattern works because it aligns with both human cognition and LLM attention mechanics.*
