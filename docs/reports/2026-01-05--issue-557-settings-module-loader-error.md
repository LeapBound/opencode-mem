...
**Date:** January 5, 2026
**Issue:** [#557](https://github.com/thedotmack/opencode-mem/issues/557) - Plugin fails to start: settings.json not generated, worker throws module loader error
...
From `/Users/alexnewman/Scripts/opencode-mem/plugin/hooks/hooks.json`:
...
From `/Users/alexnewman/Scripts/opencode-mem/scripts/build-hooks.js`:
...
From `/Users/alexnewman/Scripts/opencode-mem/src/services/worker/http/routes/SettingsRoutes.ts`:
...
From `/Users/alexnewman/Scripts/opencode-mem/src/shared/SettingsDefaultsManager.ts`:
...
4. **Check if settings.json exists:**
   ```bash
   cat ~/.opencode-mem/settings.json
   ```
...
2. **Create settings.json manually:**
   ```bash
   mkdir -p ~/.opencode-mem
   cat > ~/.opencode-mem/settings.json << 'EOF'
   {
     ...
     "OPENCODE_MEM_DATA_DIR": "$HOME/.opencode-mem",
     "OPENCODE_MEM_LOG_LEVEL": "INFO",
     "OPENCODE_MEM_MODE": "code"
   }
   EOF
   ```
