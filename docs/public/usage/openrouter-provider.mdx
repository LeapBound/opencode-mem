...
|---------|--------|---------|-------------|
| `OPENCODE_MEM_PROVIDER` | `claude`, `gemini`, `openrouter` | `claude` | AI provider for observation extraction |
| `OPENCODE_MEM_OPENROUTER_API_KEY` | string | — | Your OpenRouter API key |
| `OPENCODE_MEM_OPENROUTER_MODEL` | string | `xiaomi/mimo-v2-flash:free` | Model identifier (see list above) |
| `OPENCODE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES` | number | `20` | Max messages in conversation history |
| `OPENCODE_MEM_OPENROUTER_MAX_TOKENS` | number | `100000` | Token budget safety limit |
| `OPENCODE_MEM_OPENROUTER_SITE_URL` | string | — | Optional: URL for analytics attribution |
| `OPENCODE_MEM_OPENROUTER_APP_NAME` | string | `opencode-mem` | Optional: App name for analytics |
...
```json
{
  ...
  "OPENCODE_MEM_OPENROUTER_API_KEY": "sk-or-v1-your-key-here",
...
If OpenRouter encounters errors, opencode-mem automatically falls back to the Claude Agent SDK:
...
Either:
- Set `OPENCODE_MEM_OPENROUTER_API_KEY` in `~/.opencode-mem/settings.json`, or
...
- Reduce `OPENCODE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES`
- Reduce `OPENCODE_MEM_OPENROUTER_MAX_TOKENS`
...
HTTP-Referer: https://github.com/thedotmack/opencode-mem
X-Title: opencode-mem
