...
- Tool executions are being received (`PostToolUse: Bash(cat ~/.opencode-mem/settings.json)`)
...
The opencode-mem observation pipeline works as follows:
...
- **Core Functionality Broken:** The primary purpose of opencode-mem (persistent memory) is non-functional
...
1. Clearing the database: `rm ~/.opencode-mem/opencode-mem.db`
...
```
[INFO ] [HOOK  ] -> PostToolUse: Bash(cat ~/.opencode-mem/settings.json) {workerPort=37777}
