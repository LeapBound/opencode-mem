 ...
 * Example: npx tsx scripts/export-memories.ts "windows" windows-memories.json --project=opencode-mem
 */
...
async function exportMemories(query: string, outputFile: string, project?: string) {
  try {
    ...
    const settings = SettingsDefaultsManager.loadFromFile(join(homedir(), '.opencode-mem', 'settings.json'));
    const port = parseInt(settings.OPENCODE_MEM_WORKER_PORT, 10);
...
if (args.length < 2) {
  ...
  console.error('Example: npx tsx scripts/export-memories.ts "windows" windows-memories.json --project=opencode-mem');
