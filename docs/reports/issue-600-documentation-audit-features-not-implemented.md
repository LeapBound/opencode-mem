...
```json
{
  "OPENCODE_MEM_ENDLESS_MODE": "false",
  "OPENCODE_MEM_ENDLESS_WAIT_TIMEOUT_MS": "90000"
}
```
...
const settingKeys = [
  ...
  'OPENCODE_MEM_PROVIDER',
  'OPENCODE_MEM_GEMINI_API_KEY',
  // ... 20+ other settings
  'OPENCODE_MEM_CONTEXT_SHOW_LAST_MESSAGE',
];
```

**Neither `OPENCODE_MEM_ENDLESS_MODE` nor `OPENCODE_MEM_ENDLESS_WAIT_TIMEOUT_MS` are present in this array.**
...
|---------|---------|-------------|
| `OPENCODE_MEM_FOLDER_CLAUDEMD_ENABLED` | `false` | Enable auto-generation of folder CLAUDE.md files |
...
**The setting `OPENCODE_MEM_FOLDER_CLAUDEMD_ENABLED` is never read.** The feature runs unconditionally when files are touched.
...
| `src/shared/SettingsDefaultsManager.ts` | Add `OPENCODE_MEM_FOLDER_CLAUDEMD_ENABLED` setting |
