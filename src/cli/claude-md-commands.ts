 ...
 * - CLI: `opencode-mem generate` / `opencode-mem clean`
...
const DB_PATH = path.join(os.homedir(), '.opencode-mem', 'opencode-mem.db');
const SETTINGS_PATH = path.join(os.homedir(), '.opencode-mem', 'settings.json');
...
function formatObservationsForClaudeMd(observations: ObservationRow[], folderPath: string): string {
  ...
  lines.push('');
  lines.push('<!-- This section is auto-generated by opencode-mem. Edit content outside the tags. -->');
  lines.push('');
...
export async function generateClaudeMd(dryRun: boolean): Promise<number> {
  try {
    ...
    const observationLimit = parseInt(settings.OPENCODE_MEM_CONTEXT_OBSERVATIONS, 10) || 50;
 ...
 *
 * For each file with <opencode-mem-context> tags:
  ...
  try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            try {
              ...
              if (content.includes('<opencode-mem-context>')) {
    ...
    for (const file of filesToProcess) {
      ...
      try {
        ...
        const stripped = content.replace(/<opencode-mem-context>[\s\S]*?<\/opencode-mem-context>/g, '').trim();
