...
description: "Configure OpenCode-Mem behavior and language with the Mode System"
---

# Modes & Languages

OpenCode-Mem uses a flexible **Mode System** to adapt its behavior, observation types, and output language. This allows you to switch between different workflows (like coding vs. email investigation) or languages without reinstalling the plugin.
...
Set the active mode using the `OPENCODE_MEM_MODE` setting in `~/.opencode-mem/settings.json`:
...
```bash
export OPENCODE_MEM_MODE="code--fr"
```
