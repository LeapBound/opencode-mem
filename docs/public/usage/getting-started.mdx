...
description: "Learn how OpenCode-Mem works automatically in the background"
---

# Getting Started with OpenCode-Mem

## Automatic Operation

OpenCode-Mem works automatically once installed. No manual intervention required!
...
Every time Claude uses a tool, opencode-mem captures it:
...
Context is stored in SQLite database at `~/.opencode-mem/opencode-mem.db`.
...
OpenCode-Mem supports sessions that span multiple user prompts:
...
OpenCode-Mem provides MCP tools for querying your project history. Simply ask naturally:
