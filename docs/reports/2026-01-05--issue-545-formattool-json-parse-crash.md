...
The `formatTool` method unconditionally calls `JSON.parse()` on string inputs without error handling. When tool inputs are raw strings (not JSON), this throws an exception that propagates up the call stack, causing 400 errors for valid observation requests and silently stopping opencode-mem from recording tool usage.

## Root Cause Analysis

### Verified Issue Location

**File**: `/Users/alexnewman/Scripts/opencode-mem/src/utils/logger.ts`
**Line**: 139
**Method**: `formatTool`
