...
description: Complete reference for integrating opencode-mem worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Version:** 7.0.0 (December 2025)
**Target Audience:** Developers building opencode-mem integrations (VSCode extensions, IDE plugins, CLI tools)
</Note>
...
const DEFAULT_PORT = 37777; // Override with OPENCODE_MEM_WORKER_PORT
```
...
```bash
OPENCODE_MEM_MODEL=claude-sonnet-4-5          # Model for observations/summaries
OPENCODE_MEM_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
...
**Tag Stripping:** Removes `<private>` and `<opencode-mem-context>` tags before storage.
...
```json Response
{
  ...
  "database": {
    "path": "~/.opencode-mem/opencode-mem.db",
    "size": 1048576,
    "observations": 500,
    "sessions": 50,
    "summaries": 25
...
```json Response
{
  "projects": ["opencode-mem", "other-project", ...]
}
```
...
// Register tool
vscode.lm.registerTool('opencode-mem-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('opencode-mem', async (request, context, stream, token) => {
...
```json
{
  "name": "opencode-mem-vscode",
  "displayName": "OpenCode-Mem",
  "version": "1.0.0",
  "engines": {
    "vscode": "^1.95.0"
  },
...
describe('Worker Integration', () => {
  ...
  beforeAll(async () => {
    ...
    workerProcess = spawn('node', ['dist/worker-service.js'], {
      env: { ...process.env, OPENCODE_MEM_WORKER_PORT: '37778' }
    });
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('opencode-mem-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://opencode-mem.ai">
    Complete opencode-mem documentation
  </Card>
  <Card title="GitHub" icon="github" href="https://github.com/thedotmack/opencode-mem">
