...
1. User installs opencode-mem plugin via marketplace
...
### How opencode-mem Uses the Claude Agent SDK

Claude-mem uses `@anthropic-ai/claude-agent-sdk` (version ^0.1.76) for AI-powered operations:

**File:** `/Users/alexnewman/conductor/workspaces/opencode-mem/budapest/src/services/worker/SDKAgent.ts`
...
**File:** `/Users/alexnewman/conductor/workspaces/opencode-mem/budapest/src/services/worker-service.ts`
...
**File:** `/Users/alexnewman/conductor/workspaces/opencode-mem/budapest/src/services/infrastructure/ProcessManager.ts`
...
**File:** `/Users/alexnewman/conductor/workspaces/opencode-mem/budapest/src/services/worker/GeminiAgent.ts`

```typescript
// Line 376
const apiKey = settings.OPENCODE_MEM_GEMINI_API_KEY || process.env.GEMINI_API_KEY || '';
```

**File:** `/Users/alexnewman/conductor/workspaces/opencode-mem/budapest/src/services/worker/OpenRouterAgent.ts`

```typescript
// Line 418
const apiKey = settings.OPENCODE_MEM_OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY || '';
```
...
```
[opencode-mem] IMPORTANT: This plugin uses the Claude Agent SDK for AI operations.
...
To avoid charges, configure an alternative provider in ~/.opencode-mem/settings.json:
- Set OPENCODE_MEM_PROVIDER to "gemini" or "openrouter"
...
if (hasAnthropicKey && provider === 'claude') {
  logger.warn('SYSTEM',
    ...
    'Configure OPENCODE_MEM_PROVIDER in settings.json to use a free alternative.'
  );
}
```
...
OPENCODE_MEM_PROVIDER: 'gemini',  // Changed from 'claude' - free tier by default
```
...
```json
// ~/.opencode-mem/settings.json
{
  "OPENCODE_MEM_PROVIDER": "claude",
  "OPENCODE_MEM_ANTHROPIC_API_KEY_CONSENT": true  // New required field
}
```
...
```
[opencode-mem] Processing 25 observations
...
1. Check your provider setting: `~/.opencode-mem/settings.json`
2. Set `OPENCODE_MEM_PROVIDER` to `"gemini"` for free operation
